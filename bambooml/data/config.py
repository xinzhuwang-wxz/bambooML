...
class DataConfig(object):
    ...
    def __init__(self, print_info=True, **kwargs):
                ...
                if opts['preprocess']['params'] is None:
                    def _get(idx, default):
                        try:
                            ...
                            return default
                    params = {'length': o['length'], 'pad_mode': o.get('pad_mode', 'constant').lower(), 'center': _get(1, 'auto' if self._auto_standardization else None), 'scale': _get(2, 1), 'min': _get(3, -5), 'max': _get(4, 5), 'pad_value': _get(5, 0)}
                    if v[0] in self.preprocess_params and params != self.preprocess_params[v[0]]:
                        raise RuntimeError('Incompatible info for variable %s' % v[0])
                    if k.endswith('_mask') and params['pad_mode'] != 'constant':
                        raise RuntimeError('pad_mode must be constant for mask input %s' % k)
        ...
        if self.label_type == 'simple':
            ...
            label_exprs = ['%s' % k for k in self.label_value]
